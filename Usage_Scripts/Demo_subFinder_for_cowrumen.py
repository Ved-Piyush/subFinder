...
rumen_data = rumen_data.drop_duplicates("sequence").reset_index(drop = True)
...
for train_item in tqdm(rumen_data["sequence"].values):
...
outer_catch_df["sequence"] = rumen_data["sequence"].values
...
normalized_probs["sequence"] = rumen_data["sequence"]
...
outer_catch_df_summary.to_csv("Data/Output/Predictions/Predictions_rumen_with_probability_and_p_values_Blast_Style_new_sup_new_unsup.csv", index = False)
