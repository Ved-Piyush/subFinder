...
marine_data = marine_data.drop_duplicates("sequence").reset_index(drop = True)
...
for train_item in tqdm(marine_data["sequence"].values):
...
outer_catch_df["sequence"] = marine_data["sequence"].values
...
normalized_probs["sequence"] = marine_data["sequence"]
...
outer_catch_df_summary.to_csv("Data/Output/Predictions/Predictions_marine_with_probability_and_p_values_Blast_Style_new_sup_new_unsup.csv", index = False)
